 ...
 */
	...
	const getMap = () => {
		return new Promise((resolve, reject) => {
				...
				Object(d3_fetch__WEBPACK_IMPORTED_MODULE_2__["json"])(mapUrl).then(data => {
					$$invalidate(8, map = topojson.feature(data, data.objects[mapCollection]));
					resolve(data);
				}).catch(err => {
					reject(err);
				});
	...
	const loadingPromise = () => {
		return getMap().then(drawMap).then(getData).then(rotateMap).catch(err => {
			console.log(err);
		});
	};
	...
	$$self.$$set = $$props => {
		if ("mapUrl" in $$props) $$invalidate(3, mapUrl = $$props.mapUrl);
		if ("dataUrl" in $$props) $$invalidate(4, dataUrl = $$props.dataUrl);
		if ("mapCollection" in $$props) $$invalidate(5, mapCollection = $$props.mapCollection);
	...
	$$self.$$.update = () => {
		if ($$self.$$.dirty & /*width, margin*/ 65) {
			/**
 * REACTIVE
 */
			$: $$invalidate(12, graphWidth = width - margin.left - margin.right);
		}

		if ($$self.$$.dirty & /*height, margin*/ 66) {
			$: $$invalidate(13, graphHeight = height - margin.top - margin.bottom);
		}

		if ($$self.$$.dirty & /*margin, graphWidth, graphHeight, map*/ 12608) {
			$: $$invalidate(14, projection = Object(d3_geo__WEBPACK_IMPORTED_MODULE_3__["geoOrthographic"])().fitExtent([[margin.left, margin.top], [graphWidth, graphHeight]], map));
		}
